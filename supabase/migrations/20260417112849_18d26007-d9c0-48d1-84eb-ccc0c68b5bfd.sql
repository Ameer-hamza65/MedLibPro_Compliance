...
CREATE POLICY "Users view own search queries"
  ...
  USING (
    ...
    OR (enterprise_id IS NOT NULL AND enterprise_id = public.get_user_enterprise_id(auth.uid()))
  );
...
-- 4. Indexes to make analytics aggregations cheap
...
-- 5. Aggregate RPCs the dashboard will call
CREATE OR REPLACE FUNCTION public.analytics_activity_trend(p_bucket text DEFAULT 'day', p_days int DEFAULT 30)
RETURNS TABLE(label text, sessions bigint)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT to_char(date_trunc(p_bucket, created_at),
           CASE p_bucket WHEN 'day' THEN 'Mon DD' WHEN 'week' THEN '"Wk" IW' ELSE 'Mon YYYY' END) AS label,
...
CREATE OR REPLACE FUNCTION public.analytics_top_search_terms(p_limit int DEFAULT 12)
...
CREATE OR REPLACE FUNCTION public.analytics_title_usage(p_limit int DEFAULT 20)
